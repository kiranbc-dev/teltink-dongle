        ...
        try{
            ...
            if(isImei(bytes))
            {
                ...
                if(supposePacketLength == realPacketLength.length)
                {
                    ...
                    ByteReader byteReader = new ByteReader(unfinishedBytes);

                    int length = ByteConverter.toInt32(Arrays.copyOfRange(unfinishedBytes,4,8),0);
                    supposePacketLength = byteReader.ReadValue(length);
